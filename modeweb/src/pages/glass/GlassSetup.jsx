...
const GlassSetup = () => {
  ...
  const filteredGlassTypes = glassTypes.filter((glass) => {
    const matchesSearch =
      glass.thickness.toLowerCase().includes(searchTerm.toLowerCase()) ||
      glass.color.toLowerCase().includes(searchTerm.toLowerCase());

    const matchesThickness =
      filterThickness === "all" || glass.thickness === filterThickness;
    const matchesColor = filterColor === "all" || glass.color === filterColor;
  ...
  const confirmDelete = () => {
    setGlassTypes(glassTypes.filter((glass) => glass.id !== deleteGlass.id));
    setDeleteGlass(null);
  };
      ...
      setGlassTypes(
        glassTypes.map((glass) =>
          glass.id === editingGlass.id
            ? { ...glass, ...formData, price: parseFloat(formData.price) }
            : glass
        )
      );
    } else {
      const newGlass = {
        id: Math.max(...glassTypes.map((g) => g.id)) + 1,
        ...formData,
        price: parseFloat(formData.price),
      };
      setGlassTypes([...glassTypes, newGlass]);
    }
    setShowModal(false);
    setEditingGlass(null);
    setFormData({
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-2xl font-bold text-red-600">
                $
                {(
                  glassTypes.reduce((sum, g) => sum + g.price, 0) /
                  glassTypes.length
                ).toFixed(2)}
              </p>
            </div>
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-2xl font-bold text-yellow-600">
                {new Set(glassTypes.map((g) => g.color)).size}
              </p>
            </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">ID: {glass.id}</p>
                      </div>
                    </div>
                  </td>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full border border-gray-200">
            <div className="p-6">
              ...
              <p className="text-gray-600 text-center mb-6">
                ...
                <strong>
                  {deleteGlass.thickness} {deleteGlass.color}
                </strong>{" "}
